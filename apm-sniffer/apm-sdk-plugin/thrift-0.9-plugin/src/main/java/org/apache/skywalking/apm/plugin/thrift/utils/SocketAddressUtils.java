...
public final class SocketAddressUtils {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SocketAddressUtils.class);
    ...
    private static HostStringAccessor createHostStringAccessor() {
        try {
            ...
            LOGGER.error("[{}] {} - getHostString() not present in class InetSocketAddress.", JvmUtils.getType(),
                JvmUtils.getVersion());
    ...
    private static class ReflectiveHostStringAccessor implements HostStringAccessor {
        ...
        public String getHostString(InetSocketAddress inetSocketAddress) {
            try {
                ...
                logger.error(
                    "[" + JvmUtils.getType() + "] " + JvmUtils.getVersion() + "- Cannot access method : " + method.getName() + "",
                    e);
            } catch (InvocationTargetException e) {
                logger.error(
                    "[" + JvmUtils.getType() + "] " + JvmUtils.getVersion() + "- Error invoking method : " + method.getName() + "",
                    e);
            } catch (Exception e) {
                logger.error("[" + JvmUtils.getType() + "] " + JvmUtils.getVersion() + " - Unexpected error retrieving hostString",
                    e);
            }
            return null;
        }
    ...
    /**
     * Returns the hostname of the given {@link InetSocketAddress}, or the String form of the IP address if it does not have one.
     * Returns {@code null} if neither can be retrieved.
     *
     * <p>
     * This <b>does not</b> trigger a reverse DNS lookup if was created using an address and the hostname is not yet available.
     *
     * @param inetSocketAddress
     *            the socket address in which to retrieve the hostname from
     * @return the hostname or the String representation of the address, or {@code null} if neither can be found.
     *
     ...
     *            the value to return if neither the hostname nor the string form of the address can be found
     * @return the hostname or the String representation of the address, or the {@code defaultHostName} if neither can be found.
     *
     * @see InetSocketAddress#getHostString()
     */
    ...
    /**
     * Returns the String form of the IP address of the given {@link InetSocketAddress}, or the hostname if it has not been
     * resolved. Returns {@code null} if neither can be retrieved.
     *
     * <p>
     * This <b>does not</b> trigger a DNS lookup if the address has not been resolved yet by simply returning the hostname.
     *
     * @param inetSocketAddress
     *            the socket address in which to retrieve the address from
     * @return the String representation of the address or the hostname, or {@code null} if neither can be found.
     *
    ...
    /**
     * Returns the String form of the IP address of the given {@link InetSocketAddress}, or the hostname if it has not been
     * resolved. Returns {@code defaultAddress} if neither can be retrieved.
     *
     * <p>
     * This <b>does not</b> trigger a DNS lookup if the address has not been resolved yet by simply returning the hostname.
     *
     ...
     *            the value to return if neither the string form of the address nor the hostname can be found
     * @return the String representation of the address or the hostname, or {@code defaultAddress} if neither can be found.
     *
     * @see InetSocketAddress#isUnresolved()
     */
